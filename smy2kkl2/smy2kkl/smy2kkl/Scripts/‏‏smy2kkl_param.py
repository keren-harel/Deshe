...
def update_codes(code_string):
    new_list = []
    if code_string != None or code_string != '':
...
if __name__=='__main__':
    ...
    try:
        ...
        flag_update = arcpy.GetParameter(2)
        ...
        # Zip file of template file gdb
        zip_tmpl = os.path.join(appdata_dir, 'smy_template.gdb.zip')

        # Template file gdb
        tmpl_gdb_name = 'smy_template.gdb'
        tmpl_gdb = os.path.join(output_dir, tmpl_gdb_name)
        ...
        shutil.unpack_archive(zip_tmpl, tmpl_gdb)
        ...
        arcpy.AddMessage('>>> Delete old domains')
        old_domains = [ d.name for d in arcpy.da.ListDomains(in_gdb)]
        ...
        # Update fields of samples layer that contain list of spiecies codes
